...
def project_utilization(db: Session = Depends(get_db), _user: models.User = Depends(get_current_user)):
    rows = (
        db.query(
            models.Project.name,
            func.count(models.SeatAllocation.id.distinct()).label("occupied_seats"),
